...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
     ...
     */
    def always[T](value: T): Future[T] = Promise[T].success(value).future // Promise is completed with the value
    
    /** Returns a future that is never completed.
     *
     *  This future may be useful when testing if timeout logic works correctly.
     */
    def never[T]: Future[T] = Promise[T].future  // No value is ever set on the promise
     ...
     */
    def all[T](fs: List[Future[T]]): Future[List[T]] = Future.sequence(fs) // List of futures is merged into a single future
     ...
     */
    def any[T](fs: List[Future[T]]): Future[T] = Future.firstCompletedOf(fs) // Returns the first future that completes
